$(function () {
    ...
    $.ajax({
        ...
        success: ((data) => {
            ...
            let lastYearData = [];
            for (let i = 0; i < 12; i++) {
                lastYearData[i] = data[i].cnt
            }

            let thisYearData = [];
            for (let i = 0; i < 12; i++) {
                thisYearData[i] = data[i + 12].cnt
            }
    ...
    $.ajax({
        type: 'get',
        url: '/cust-mgt/anal-visit',
        dataType: "JSON",
        success: ((data) => {
            let labelData = [];
            for (let i = 0; i < data.length; i++) {
                labelData[i] = data[i].puppy_species_nm;
            }

            let datasetsData = [];
            for (let i = 0; i < data.length; i++) {
                datasetsData[i] = data[i].count
            }
            let randomColor = [];
            for (let i = 0; i < data.length; i++) {
